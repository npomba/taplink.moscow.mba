export default {
  title: {
    ru: 'Об академии',
    en: 'About',
  },

  aboutTitle: {
    ru: 'ОБ АКАДЕМИИ',
    en: 'ABOUT',
  },

  teachersTitleLabel: {
    ru: 'ПРЕПОДАВАТЕЛИ',
    en: 'EXPERTS',
  },

  teachersTitleFirst: {
    ru: 'Российские и',
    en: 'Russian and',
  },

  teachersTitleRed: {
    ru: 'зарубежные',
    en: 'foreign',
  },

  teachersTitleSecond: {
    ru: 'эксперты программы',
    en: 'experts of the program',
  },

  teachersDics: {
    ru:
      'Перенимайте уникальный опыт международных экспертов, адаптированный под рынок РФ',
    en:
      'Learn from the unique foreign experts who adapted their experience for Russian market',
  },

  teachersListItemTitle: {
    ru: 'Практикующие специалисты',
    en: 'Practitioners',
  },

  teachersListItemDisc: {
    ru: 'Реализовывали крупные проекты на рынках Европы и США',
    en: 'They have implemented major projects on the markets of Europe and USA',
  },

  teachersListItemTitleSecond: {
    ru: 'Прошли многоэтапную проверку',
    en: 'Multi-stage verification',
  },

  teachersListItemDiscSecond: {
    ru:
      'Прошли многоэтапную проверку Moscow Academy и имеют аккредитацию на преподавание',
    en:
      'They have passed Moscow Academy’s multi-stage verification and have teaching accreditation',
  },

  teachersListItemTitleThird: {
    ru: 'Международный опыт преподавания',
    en: 'International teaching experience',
  },

  teachersListItemDiscThird: {
    ru: 'Преподают в ведущих бизнес-школах мира',
    en: 'They work in the leading world-class business schools',
  },

  teachersProsTitle: {
    ru: 'Более 150 профессоров международного уровня',
    en: 'More than 150 international-level professors',
  },

  teachersTeacherOneTitle: {
    ru: 'Баранова Татьяна',
    en: 'Baranova Tatyana',
  },

  teachersTeacherOneDics: {
    ru:
      'Эксперт по деловому этикету и протоколу. «Про ЭТИКЕТ», основатель образовательного проекта',
    en: '',
  },

  teachersTeacherTwoTitle: {
    ru: 'Ряковский Сергей',
    en: 'Ryakovskiy Sergey',
  },

  teachersTeacherTwoDics: {
    ru:
      'Эксперт по стратегическому менеджменту. Автор многочисленных пособий по управлению персоналом',
    en: '',
  },

  teachersTeacherThreeTitle: {
    ru: 'Коноплянский Дмитрий',
    en: 'Konoplyanskiy Dmitriy',
  },

  teachersTeacherThreeDics: {
    ru:
      'Основатель сети ювелирных салонов в Москве и регионах. Советник группы «НЛМК», «НК РОСНЕФТЬ»',
    en: '',
  },

  teachersTeacherFourTitle: {
    ru: 'Борисов Александр',
    en: 'Borisov Aleksandr',
  },

  teachersTeacherFourDics: {
    ru: 'Эксперт по бизнес-планированию, инвестиционному и финансовому анализу',
    en: '',
  },

  teachersTeacherFiveTitle: {
    ru: 'Александр Додерер',
    en: 'Aleksandr Doderer',
  },

  teachersTeacherFiveDics: {
    ru:
      'Основатель и глава агентства по стратегическим коммуникациям GRUPPE DREI.',
    en: '',
  },

  teachersTeacherSixTitle: {
    ru: 'Янник Траншье',
    en: 'Yannik Transhye',
  },

  teachersTeacherSixDics: {
    ru:
      'Эксперт в сфере инновационного менеджмента, технологический брокер, предприниматель',
    en: '',
  },

  teachersTeacherSevenTitle: {
    ru: 'Александр Иванов',
    en: 'Aleksandr Ivanov',
  },

  teachersTeacherSevenDics: {
    ru:
      'ОПТИМУМ, генеральный директор. HEINZ, региональный директор по производству (7 заводов)',
    en: '',
  },

  teachersTeacherEightTitle: {
    ru: 'Дубовик Сергей',
    en: 'Dubovyk Serhey',
  },

  teachersTeacherEightDics: {
    ru: 'Специалист в области продаж и управления коммерческой деятельностью.',
    en: '',
  },

  teachersCtaBtn: {
    ru: 'Запросить полный список',
    en: 'Request full list',
  },

  teachersBiographyLink: {
    ru: 'Биография',
    en: 'Biography',
  },

  teachersPopupFormTitle: {
    ru: 'Узнайте своих преподавателей',
    en: 'Get to know experts ',
  },

  teachersPopupFormDics: {
    ru:
      'Оставьте заявку и получите консультацию по преподавателям, программам MBA, а также узнайте возможные варианты скидок и требования к поступлению',
    en:
      'Submit a request and receive a consultation on experts, programs, discounts, and requirements',
  },

  uptodateTitle: {
    ru: 'Только актуальный контент',
    en: 'Up to Date content only',
  },

  uptodateDicsFirst: {
    ru:
      'MBAcademy записывает контент очных лекций, поэтому это хорошая возможность',
    en:
      'MBAcademy records its face-to-face lectures so that the students could',
  },

  uptodateDicsRed: {
    ru: 'получить MBA',
    en: 'get education',
  },

  uptodateDicsSecond: {
    ru: 'не посещая наши кампусы',
    en: 'without personal presence in our campuses',
  },

  uptodateBenefitOneDics: {
    ru: 'Новейшая программа',
    en: 'The newest program of',
  },

  uptodateYear: {
    ru: 'год',
    en: 'year',
  },

  uptodateYearV2: {
    ru: 'года',
    en: '',
  },

  uptodateBenefitTwoTitle: {
    ru: 'эксперта',
    en: 'experts',
  },

  uptodateBenefitTwoDics: {
    ru: 'международных экспертов',
    en: 'international experts',
  },

  uptodateBenefitThreeTitle: {
    ru: 'ТОП 3',
    en: 'TOP 3',
  },

  uptodateBenefitThreeDics: {
    ru: 'международных экспертов',
    en: 'international experts',
  },

  accreditationTitle: {
    ru: 'Аккредитации',
    en: 'Accreditation',
  },

  accreditationDics: {
    ru:
      'В России Moscow Business Academy имеет все лицензии и аккредитации, позволяющие готовить специалистов международного уровня по направлению MBA',
    en:
      'Moscow Business Academy has all needed licenses and accreditation to graduate world-class MBA specialists',
  },

  accreditationItemOne: {
    ru:
      'В России Moscow Business Academy имеет все лицензии и аккредитации, позволяющие готовить специалистов международного уровня по направлению MBA',
    en:
      'Moscow Business Academy has all needed licenses and accreditation to graduate world-class MBA specialists',
  },

  accreditationItemTwo: {
    ru:
      'Также Moscow Business Academy является членом Российской Ассоциации Бизнес Образования и имеет государственную лицензию, что подтверждает полное соответствие всех предоставляемых нами программ образовательному стандарту РФ',
    en:
      'Moscow Business Academy is also a member of Russian Association of Business Education and has a state license, which proves that our programs fully comply with the educational standards of Russian Federation',
  },

  accreditationLicenseLink: {
    ru: 'Лицензия Министерства Образования серия ... № ....',
    en: 'License of the Ministry of Education ... № ....',
  },
}
